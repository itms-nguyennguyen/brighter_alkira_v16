...
export class HMSPatientDocumentsKanbanRecord extends KanbanRecord {
    setup() {
        super.setup();
    ...
    onGlobalClick(ev) {
        ...
        if (ev.target.classList.contains("o_mrp_download")) {
            window.location = `/web/content/patient.document/${this.props.record.resId}/datas?download=true`;
            return;
