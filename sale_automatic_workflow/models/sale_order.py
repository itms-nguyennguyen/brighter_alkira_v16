...
class SaleOrder(models.Model):
    ...
    workflow_process_id = fields.Many2one(
        ...
        ondelete="restrict",
        default=lambda self: self._get_default_workflow(),
    )
    ...
    def _get_default_workflow(self):
        #default xml id = automatic_validation if available, if not, return false
        autoworkflow = self.env.ref("sale_automatic_workflow.automatic_validation", raise_if_not_found=False)
        if autoworkflow:
            return autoworkflow.id
        return False
