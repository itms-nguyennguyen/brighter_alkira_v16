...
TWILIO_GET_ACCOUNT_DETAILS_ENDPOINT = TWILIO_BASE_URL_ENDPOINT + "/2010-04-01/Accounts/%s" + TWILIO_RESPONSE_ENDPOINT
TWILIO_GET_MESSAGES_STATUS_ENDPOINT = (TWILIO_BASE_URL_ENDPOINT + "/2010-04-01/Accounts/%s/Messages/%s" +
                                       TWILIO_RESPONSE_ENDPOINT)
...
class TwilioSendSMSAPI():
    ...
    def test_twilio_sms_connection_api(self, twilio_account):
        ...
        try:
            ...
            error_msg = _("Something went wrong while calling Send SMS API.\nRequested URL: %s\nError: %s" % (
                TWILIO_SEND_SMS_URL_ENDPOINT_FINAL, e))
            raise UserError(error_msg)

    def get_twilio_sms_account_details_api(self, twilio_account):
        base64_auth = self._get_authorization_token(twilio_account)
        TWILIO_GET_ACCOUNT_DETAILS_ENDPOINT_FINAL = TWILIO_GET_ACCOUNT_DETAILS_ENDPOINT % twilio_account.account_sid
        try:
            headers = {
                "Content-Type": "application/x-www-form-urlencoded",
                "Authorization": "Basic %s" % base64_auth,
            }
            response = requests.get(TWILIO_GET_ACCOUNT_DETAILS_ENDPOINT_FINAL, headers=headers, timeout=20)
            # response.raise_for_status()
            return response
        except Exception as e:
            error_msg = _(
                "Something went wrong while calling GET Account Details SMS API.\nRequested URL: %s\nError: %s" % (
                    TWILIO_GET_ACCOUNT_DETAILS_ENDPOINT_FINAL, e))
            raise UserError(error_msg)
    ...
    def post_twilio_sms_send_to_recipients_api(self, twilio_account, datas):
        ...
        TWILIO_SEND_SMS_URL_ENDPOINT_FINAL = ""
        try:
            TWILIO_SEND_SMS_URL_ENDPOINT_FINAL = TWILIO_SEND_SMS_URL_ENDPOINT % twilio_account.account_sid
    ...
    def get_retrieve_messages_status_from_twilio_api(self, twilio_account, message_sid):
        base64_auth = self._get_authorization_token(twilio_account)
        headers = {
            "Content-Type": "application/x-www-form-urlencoded",
            "Authorization": "Basic %s" % base64_auth
        }
        final_req_url = TWILIO_GET_MESSAGES_STATUS_ENDPOINT % (twilio_account.account_sid, message_sid)
        try:
            response_obj = requests.get(final_req_url, headers=headers, timeout=20)
            return response_obj
        except Exception as e:
            error_msg = _("Something went wrong while calling Get Messages Status.\nRequested URL: %s\nError: %s" % (
                final_req_url, e))
            raise UserError(error_msg)
