...
class TwilioSendSmsGatewayAccount(models.Model):
    ...
    def _compute_total_sms_data(self):
        ...
        for twilio_account in self:
            twilio_account.sms_records_count = len(
                twilio_sms_send_obj.search([('twilio_account_id', '=', twilio_account.id)]))
            twilio_account.account_sms_logs_count = len(
                twilio_sms_log_history_obj.search([('twilio_account_id', '=', twilio_account.id)]))

    def _compute_total_cron(self):
        for twilio_account in self:
            twilio_cron = self.env.ref("twilio_sms_gateway_gsc.ir_cron_auto_retrieve_messages_status_from_twilio")
            twilio_account.account_cron = twilio_cron and len(twilio_cron) or 0
    ...
    account_from_mobile_number = fields.Char("From Mobile Number", help="Mobile number should be with country code.",
    ...
    sms_so_confirm_template_id = fields.Many2one("twilio.sms.template", "SMS Template",
                                                 domain="[('model_id', '!=', False), ('model_id.model', '=', 'sale.order')]",
                                                 copy=False)
    is_validate_do_to_send_sms = fields.Boolean("Validate Delivery to Send SMS?", default=False, copy=False)
    sms_do_validate_template_id = fields.Many2one("twilio.sms.template", "SMS Template",
                                                  domain="[('model_id', '!=', False), ('model_id.model', '=', 'stock.picking')]",
                                                  copy=False)
    ...
    account_cron = fields.Integer(string='Cron Count', compute='_compute_total_cron')
    ...
    def _check_duplicate_twilio_account(self):
        ...
        if twilio_accounts and len(twilio_accounts) > 1:
            raise ValidationError(
                _('You can not create same duplicate Twilio Account with same Account SID and Authtoken.'))
        return True
    ...
    def action_open_sms_account_logs_records(self):
        """
            :return: action or error
        """
        account_sms_logs_records = self.env['twilio.sms.log.history'].sudo().search(
    ...
    def action_open_account_cron_records(self):
        twilio_cron = self.env.ref("twilio_sms_gateway_gsc.ir_cron_auto_retrieve_messages_status_from_twilio")
        if not twilio_cron:
            return True
        action = {
            'domain': "[('id', 'in', " + str(twilio_cron.ids) + " )]",
            'name': "Twilio Automations",
            'view_mode': 'tree,form',
            'res_model': 'ir.cron',
            'type': 'ir.actions.act_window',
        }
        ...
        if response_obj.status_code in [200, 201]:
            ...
            if isinstance(response, dict) and response.get('status') in SUCCESS_RESPONSE_STATUES:
                if method_call == "test_and_confirm_twilio_sms_account":
                    ...
                    self._cr.commit()
                error_msg = _("Service working properly!!! Your message sent successfully to %s" % (
            ...
            error_msg = _(
                "Due to below reason your message is not sent to %s.\nResponse Error,\n Response Status Code: %s\n Response Message: %s\n Error Code: %s\n More Info.: %s" % (
                    self.test_connection_mobile_number, status_code, message, code, more_info))
            raise UserError(error_msg)
        else:
            error_msg = _(
                "Something went to wrong!!!.\nResponse Status Code: %s\nError More Info.: https://www.twilio.com/docs/api/errors" % (
    ...
    def get_twilio_account_details(self):
        ...
        if response_obj.status_code in [200, 201]:
            ...
            status_code = response.get("status")
            error_msg = _(
                "Due to below reason not get account details from Twilio.\nResponse Error,\n Response Status Code: %s\n Response Message: %s\n Error Code: %s\n More Info.: %s" % (
                    status_code, message, code, more_info))
            raise UserError(error_msg)
        else:
            error_msg = _(
                "Something went to wrong!!!.\nResponse Status Code: %s\nError More Info.: https://www.twilio.com/docs/api/errors" % (
    ...
    def auto_retrieve_messages_status_from_twilio(self, args=None):
        """Retrieve Messages Status from Twilio
        ...
        # Collect Message Logs with status is Queued
        _domain = [("status", "=", "queued")]
        messages_logs = self.env['twilio.sms.log.history'].search(_domain, order="id asc")
        if not messages_logs:
            return True

        # Call API & Process Response
        TwilioSendSMSAPIObj = TwilioSendSMSAPI()
        for msg_log in messages_logs:
            response_obj = TwilioSendSMSAPIObj.get_retrieve_messages_status_from_twilio_api(msg_log.twilio_account_id,
                                                                                            msg_log.message_id)
            if response_obj.status_code in [200, 201]:
                response = response_obj.json()
                status = response.get("status")
                failed_code = response.get("error_code")
                msg_log.write({
                    "status": status,
                    "message_failed_code": failed_code
                })
            elif response_obj.status_code in [400, 401, 404]:
                response = response_obj.json()
                msg_log.write({
                    'error_code': response.get('code'),
                    'error_message': response.get('message'),
                ...
                _logger.warning(
                    "Due to any reason we are not able to retrieve message status.\nResponse Status Code: %s" %
                    response_obj.status_code)
            self._cr.commit()
