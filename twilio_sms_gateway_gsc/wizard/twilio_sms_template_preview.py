...
class TwilioSMSTemplatePreview(models.TransientModel):
    ...
    sms_template_id = fields.Many2one('twilio.sms.template', required=True, domain="[('model_id', '=', False)]",
                                      ondelete='cascade')
    ...
    def _compute_sms_template_fields(self):
        for wizard in self:
            if wizard.sms_template_id and wizard.resource_ref:
                wizard.message = \
                wizard.sms_template_id._render_field('message', [wizard.resource_ref.id], set_lang=wizard.lang)[
